...
from Graph_Obj_Fill import color_fill
...
def segment_img(img):
    # img = cv2.imread('/home/komaldadhich/Desktop/scatter_2_1_lg.png', cv2.IMREAD_UNCHANGED)
    # if alpha channel is having zero it represents no color
    if(img.shape[2]==4):
        img[img[:,:,3]==0] = [255,255,255,255]
        img = cv2.cvtColor(img, cv2.COLOR_RGBA2RGB)
    # t = cv2.resize(img, (650,500), interpolation = cv2.INTER_AREA)
    ...
    sure_fg_inv = inverte(sure_fg)
    sure_bg_inv = inverte(sure_bg)
    # plt.figure("foreground image")
    # plt.imshow(sure_fg, cmap='gray')
    # plt.show()

    # plt.figure("background image")
    # plt.imshow(sure_bg_inv, cmap='gray')
    # plt.show()
    ...
    # img = cv2.resize(img, (650,500), interpolation = cv2.INTER_AREA)
    # cv2.imshow("Segmented Result",img)
