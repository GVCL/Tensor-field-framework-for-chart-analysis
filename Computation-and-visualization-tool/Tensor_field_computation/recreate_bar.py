...
def new_list(x, y, cords):
    ...
    for i in range(1, 6):
        ...
        if i == 1:
            ...
            if r in cords and r not in n_list:
                n_list.append(r)
            if u in cords and u not in n_list:
                n_list.append(u)
            if v in cords and v not in n_list:
                n_list.append(v)
            if s in cords and s not in n_list:
                n_list.append(s)


        if p in cords and p not in n_list:
            n_list.append(p)
        if q in cords and q not in n_list:
            n_list.append(q)
        if t in cords and t not in n_list:
            n_list.append(t)
        if w in cords and w not in n_list:
            n_list.append(w)

    print("nlist",x, y, n_list, len(n_list))
    return n_list
...
data_tensors = pd.read_csv("/home/komaldadhich/Documents/study/Research/graph_percept/source/project/chart-digitizer/Computation-and-visualization-tool/Tensor_field_computation/tensor_vote_matrix.csv", sep=",", index_col=False)
...
for i in range(X*Y):
    if (data_tensors['CL'][i]!=0.0 or data_tensors['CP'][i]!=0) and data_tensors['CL'][i]<=0.05:
...
for i in cord_list1:
    x = i[0]
    y = i[1]
    n_list = new_list(x , y, cord_list['cord_val'])
    x_n =0
    y_n =0
    ...
    if (len(n_list) == 7):
        for p in n_list:
            ...
            y_n += p[1]
        if (len(n_list) != 0):
            cord_list_x.append(int(x_n//(len(n_list)+1)))
            cord_list_y.append(int(y_n//(len(n_list)+1)))
        else:
            cord_list_x.append(int(x_n))
            cord_list_y.append(int(y_n))

# plt.scatter(cord_list['x_val'], cord_list['y_val'], s=5)
# plt.show()
print("ag y",cord_list_y)
