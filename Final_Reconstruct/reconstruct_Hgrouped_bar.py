...
path = "/Users/daggubatisirichandana/PycharmProjects/chart_percept/Final_Reconstruct/Generated_data/H_grouped_bar/h_gb03/"
...
while i<len(unused_centers):
    ...
    for j in range(i+1,len(unused_centers)):
        print(unused_centers[i],unused_centers[j])
        print(unused_centers[j][0]-unused_centers[i][0],abs(unused_centers[j][1]-unused_centers[i][1]))
    ...
    if flag == False:
        ...
        print("++++",abs(bar_heights[len(bar_heights)-1]+base_val-unused_centers[i][1]),abs(abs(unused_centers[i][0]-bar_centers[len(bar_centers)-1])-(3*bar_width/2)))
